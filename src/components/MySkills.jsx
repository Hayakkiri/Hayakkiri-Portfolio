...
function MySkills() {
	...
	const skillCategories = [
		...
		},
		{
			...
			skills: ["Inspection", "QC Tools", "Documentation"],
			icon: (
				<svg
					className="w-8 h-8"
					fill="none"
		...
		},
		{
			...
			skills: ["Technical Drawings", "2D & 3D", "GD & T"],
			icon: (
				<svg
					className="w-8 h-8"
					fill="none"
		...
		},
		{
			id: 2,
			title: "CNC Operating",
			skills: ["Basic Operating", "Drawing Interpretation", "Tool Setup"],
			icon: (
				<svg
					className="w-8 h-8"
					fill="none"
		...
		>
			<div className="max-w-7xl mx-auto px-8">
				...
				>
					...
					<div
						className={`w-24 h-1 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 mx-auto rounded-full transform transition-all duration-800 delay-200 ${
							isVisible
					...
					{skillCategories.map((category, index) => (
						<div
							...
							className={`${category.bgColor} ${
								...
								category.hoverColor
							} border-2 rounded-2xl p-6 transform transition-all duration-800 hover:scale-105 hover:-translate-y-2 hover:shadow-2xl cursor-pointer group ${
								isVisible
									? "translate-y-0 opacity-100"
									: "translate-y-10 opacity-0"
							}`}
						...
						>
							...
							<div
								className={`w-16 h-16 bg-gradient-to-r ${
									category.color
								} rounded-xl flex items-center justify-center text-white mb-4 mx-auto transform transition-all duration-300 ${
							...
							<div
								className={`absolute inset-0 bg-gradient-to-r ${category.color} opacity-0 group-hover:opacity-10 rounded-2xl transition-opacity duration-300 pointer-events-none`}
							></div>
						</div>
					))}
				</div>
				...
				<div
					className={`bg-white/70 backdrop-blur-md rounded-3xl p-8 shadow-2xl border border-white/20 transform transition-all duration-1000 delay-1000 ${
						isVisible
							? "translate-y-0 opacity-100"
							: "translate-y-10 opacity-0"
					}`}
				>
					<h3
						className={`text-2xl md:text-3xl font-bold text-center bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent mb-8 transform transition-all duration-800 delay-1200 ${
							isVisible
								? "scale-100 opacity-100"
								: "scale-95 opacity-0"
						}`}
						...
						{proficiencyLevels.map((item, index) => (
							...
							>
								<div className="flex justify-between items-center mb-3">
									...
									<span
										className={`text-sm font-bold px-3 py-1 rounded-full bg-gradient-to-r ${item.color} text-white shadow-lg`}
									>
										{item.level}%
									</span>
								</div>
				...
				{/* Additional Info */}
				<div
					className={`text-center mt-12 transform transition-all duration-1000 delay-2000 ${
						isVisible
							? "translate-y-0 opacity-100"
							: "translate-y-5 opacity-0"
					}`}
				>
					<p
						className={`text-gray-600 transform transition-all duration-800 delay-2100 ${
							isVisible
								? "scale-100 opacity-100"
								: "scale-95 opacity-0"
						}`}
					>
						Continuously expanding expertise through hands-on
